...
map<string, int> countWords(vector<string> magazine){

    map <string, int> words;

    for (unsigned int i=0; i<magazine.size(); i++){
	if (words.count(magazine[i]) > 0){
	    words.at(magazine[i])++;
	} else {
	    words.insert(pair<string, int>(magazine[i], 1));
	}
    }
    return words;
}
...
void checkMagazine(vector<string> magazine, vector<string> note) {
    ...
    map <string, int> words;
    words = countWords(magazine);

    for (unsigned int i = 0; i<note.size(); i++){
	if (words.count(note[i])>0){
	    if (words.at(note[i]) != 0){
		words.at(note[i])--;
	    } else {
		cout << "No" << endl;
		return;
